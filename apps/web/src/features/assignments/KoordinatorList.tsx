...
import { useKoordinatorAssignments, useDeleteKoordinatorAssignment } from '../../hooks/useAssignments';
...
import { KoordinatorFormModal } from './KoordinatorFormModal';
import { Plus, Trash2 } from 'lucide-react';

export const KoordinatorList = () => {
  ...
  const { data, isLoading, isError, refetch } = useKoordinatorAssignments(page, 15, semesterFilter);
  const { data: semestersData } = useSemesters(1, 100);
  
  const deleteMutation = useDeleteKoordinatorAssignment();
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h2 className="text-2xl font-bold text-gray-900">Koordinator Assignments</h2>
      ...
      {isLoading ? (
        ...
        <>
          <Table columns={['Course', 'Semester', 'Koordinator', 'Status', 'Created At', ...(isSuperAdmin ? ['Actions'] : [])]}>
      ...
      <KoordinatorFormModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
      
      <ConfirmDialog
        isOpen={deleteId !== null}
        title="Delete Assignment"
        message="Are you sure you want to delete this Koordinator assignment? This action cannot be undone."
        confirmText="Delete"
